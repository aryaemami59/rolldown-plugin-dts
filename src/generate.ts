...
/**
 * An empty ES module stub (`export {}`) used as replacement code for modules
 * that should emit nothing, e.g. non-declaration chunks when
 * {@linkcode OptionsResolved.emitDtsOnly | emitDtsOnly} is enabled.
 */
export const EMPTY_STUB = `export {}`

/**
 * A single source module tracked during `.d.ts` generation, holding its raw
 * source alongside the metadata needed to decide whether and how to emit a
 * declaration file for it. Stored as the value type of a {@linkcode DtsMap}.
 */
export interface TsModule {
  ...
  /**
   * Whether the source module is a JavaScript file (as opposed to a
   * TypeScript file), which affects whether an existing `.d.ts` on disk is
   * reused instead of generating one.
   */
  jsFile: boolean
}
...
/**
 * Creates the {@linkcode Plugin | Rolldown plugin} responsible for generating
 ...
 * @returns A {@linkcode Plugin | Rolldown plugin} that registers the hooks needed for `.d.ts` generation.
 */
export function createGeneratePlugin(
  ...
  /**
   * A map of resolved input id to its output name alias (the key in
   * Rolldown's `input` object), used to name the emitted `.d.ts` chunk.
   *
   * @example
...
/**
 * A handle to a forked `tsc` worker process, used when
 * {@linkcode OptionsResolved.parallel | parallel} is enabled.
 */
interface TscWorker {
  /**
   * Sends an emit request to the worker process and resolves with its
   * result.
   */
  emit: (options: TscOptions) => Promise<TscResult>

  /**
   * Kills the worker child process.
   */
  kill: () => void
}

/**
 * Forks the `tsc` worker process and wires up request/response message
 * passing keyed by sequential ids. Pending requests are rejected if the
 * worker exits before responding.
 *
 * @returns A {@linkcode TscWorker} handle for sending emit requests to the worker process.
 */
