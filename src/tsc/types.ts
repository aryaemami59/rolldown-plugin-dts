import type { Options, OptionsResolved, resolveOptions } from '../options.ts'
 ...
 * {@linkcode OptionsResolved} after {@linkcode resolveOptions()} has been
 * applied.
 */
export interface TscOptions
  extends
    Required<
      ...
      >
    >,
    Pick<Options, 'tsconfig' | 'vue'> {
  /**
   * Narrows the inherited `tsconfig` type from `string | boolean | undefined`
   * to `string | undefined`. By this point {@linkcode resolveOptions()} has
   * already converted `false` to `undefined` and resolved any relative path.
   */
  tsconfig?: string
